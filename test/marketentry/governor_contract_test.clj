  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Pitons Marine Services Ltd"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Pitons Marine Services Ltd" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest suspension-disqualifying-is-held-and-unoverridable
  (testing "an active suspension window under s.114 -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:suspension-disqualifying} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))

(deftest suspension-invalid-duration-is-held
  (testing "a :suspension-duration-months below s.114(3)'s 6-month statutory floor -> HARD hold, regardless of the apparent window"
    (let [[db actor] (fresh)
...
(deftest suspension-expired-before-submission-is-clean
  (testing "a suspension whose computed window has already lapsed before submission-date -> NOT disqualifying, submit proceeds to the usual actuation escalation"
    (let [[db actor] (fresh)
...
(deftest tax-registration-unverified-is-held-and-unoverridable
  (testing "unverified tax registration (Inland Revenue Department) when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:tax-registration-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
