(ns marketentry.marketentryllm
  ...
  the Saint Lucia (LCA) public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  :actuation/submit-filing` -- real-world Central Public Procurement
  Board portal submission."
  [db {:keys [subject]}]
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "suspension-start-date=" (:suspension-start-date e)
                        " suspension-duration-months=" (:suspension-duration-months e)
