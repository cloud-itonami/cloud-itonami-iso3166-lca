...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (LCA, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up suspension-disqualifying) ==")
    ...
    (println "== filing/submit eng-4 (suspension-disqualifying, active window -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up tax-registration-unverified) ==")
    ...
    (println "== filing/submit eng-5 (tax-registration-unverified -> HARD hold) ==")
    (println (exec-op actor "t11" {:op :filing/submit :subject "eng-5"} operator))

    (println "== jurisdiction/assess eng-6 (suspension already expired -- clean) ==")
    ...
    (println "== filing/submit eng-6 (suspension expired long before submission-date -> escalates, clean) ==")
    (let [r (exec-op actor "t13" {:op :filing/submit :subject "eng-6"} operator)]
      (println r)
      (println (approve! actor "t13")))

    (println "== jurisdiction/assess eng-7 (sets up invalid suspension-duration) ==")
    ...
    (println "== filing/submit eng-7 (suspension-duration-months below s.114(3)'s 6-month floor -> HARD hold) ==")
