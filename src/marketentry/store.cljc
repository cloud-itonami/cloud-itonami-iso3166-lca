(ns marketentry.store
  "SSoT for the Saint Lucia (LCA) market-entry compliance actor, behind
  a `Store` protocol so the backend is a swap, not a rewrite -- the
  same seam every prior cloud-itonami actor in this fleet uses.
  ...
  `:suspension-start-date` / `:suspension-duration-months` /
  `:submission-date` are ground truth for the
  `suspension-disqualifying?` flagship check (Public Procurement Act,
  Cap. 15.10 s.114 + Public Procurement Regulations 2023 reg.25(2)(b)).
...
(defn demo-data
  ...
  `:suspension-start-date` / `:suspension-duration-months` /
  `:submission-date` are ground truth for the `suspension-
  disqualifying?` flagship check (s.114 + reg.25(2)(b))."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Pitons Marine Services Ltd" :portal "finance.gov.lc/tenders"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :suspension-start-date nil :suspension-duration-months nil :submission-date "2026-07-23"
             :requires-tax-registration? true :tax-registration-verified? true
             :drafted? false :submitted? false
             :jurisdiction "LCA" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "finance.gov.lc/tenders"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :suspension-start-date nil :suspension-duration-months nil :submission-date "2026-07-23"
             :requires-tax-registration? true :tax-registration-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Castries Trading Co" :portal "finance.gov.lc/tenders"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "LCA" :status :intake}
    "eng-4" {:id "eng-4" :operator "Vieux Fort Export Ltd" :portal "finance.gov.lc/tenders"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :suspension-start-date "2026-03-01" :suspension-duration-months 6 :submission-date "2026-07-23"
             :requires-tax-registration? true :tax-registration-verified? true
             :drafted? false :submitted? false
             :jurisdiction "LCA" :status :intake}
    "eng-5" {:id "eng-5" :operator "Gros Islet Logistics" :portal "finance.gov.lc/tenders"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :suspension-start-date nil :suspension-duration-months nil :submission-date "2026-07-23"
             :requires-tax-registration? true :tax-registration-verified? false
             :drafted? false :submitted? false
             :jurisdiction "LCA" :status :intake}
    "eng-6" {:id "eng-6" :operator "Soufriere Builders Ltd" :portal "finance.gov.lc/tenders"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             ;; a LAWFUL 6-month suspension (s.114(3)'s own statutory
             ;; floor) whose computed window (2025-06-01 .. 2025-12-01)
             ;; has already lapsed well before :submission-date -- NOT
             ;; disqualifying, unlike eng-4's still-active window.
             :suspension-start-date "2025-06-01" :suspension-duration-months 6 :submission-date "2026-07-23"
             :requires-tax-registration? true :tax-registration-verified? true
             :drafted? false :submitted? false
             :jurisdiction "LCA" :status :intake}
    "eng-7" {:id "eng-7" :operator "Anse La Raye Fisheries Co-op" :portal "finance.gov.lc/tenders"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             ;; :suspension-duration-months (2) is BELOW s.114(3)'s own
             ;; 6-month statutory floor -- an untrustworthy record, held
             ;; regardless of how long ago :suspension-start-date was
             ;; (the apparent window would already have lapsed by
             ;; :submission-date if the declared duration were taken at
             ;; face value -- this demonstrates the invalid-duration leg
             ;; fires independently of the window-overlap leg).
             :suspension-start-date "2020-01-01" :suspension-duration-months 2 :submission-date "2026-07-23"
             :requires-tax-registration? true :tax-registration-verified? true
             :drafted? false :submitted? false
             :jurisdiction "LCA" :status :intake}}})
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               suspension-start-date suspension-duration-months submission-date
                               requires-tax-registration? tax-registration-verified?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    suspension-start-date                 (assoc :engagement/suspension-start-date suspension-start-date)
    suspension-duration-months            (assoc :engagement/suspension-duration-months suspension-duration-months)
    submission-date                       (assoc :engagement/submission-date submission-date)
    (some? requires-tax-registration?)    (assoc :engagement/requires-tax-registration? requires-tax-registration?)
    (some? tax-registration-verified?)    (assoc :engagement/tax-registration-verified? tax-registration-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/suspension-start-date :engagement/suspension-duration-months :engagement/submission-date
   :engagement/requires-tax-registration? :engagement/tax-registration-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :suspension-start-date (:engagement/suspension-start-date m)
     :suspension-duration-months (:engagement/suspension-duration-months m)
     :submission-date (:engagement/submission-date m)
     :requires-tax-registration? (boolean (:engagement/requires-tax-registration? m))
     :tax-registration-verified? (boolean (:engagement/tax-registration-verified? m))
